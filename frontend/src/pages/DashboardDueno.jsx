...
const STAT_CARDS = [
  { icon: 'box', iconBg: '#f1f5f9', value: '120', label: 'Productos totales', trend: '+5 este mes', trendType: 'positive' },
  { icon: 'alert', iconBg: '#fff1f2', value: '8', label: 'Stock Bajo', trend: '3 críticos', trendType: 'negative' },
  { icon: 'dollar', iconBg: '#f0fdf4', value: '$1,200', label: 'Ventas hoy', trend: '+12% vs ayer', trendType: 'positive' },
  { icon: 'tag', iconBg: '#f8fafc', value: '6', label: 'Categorías', trend: 'Sin cambios', trendType: 'neutral' },
];
...
function DashboardDueno() {
  ...
  useEffect(() => {
    ...
    // Ejemplo: dashboardService.getResumenDueno().then(setDatosDashboard);
  }, []);
      ...
      <main className="dueno-content">
        ...
        <section className="dueno-stats-grid">
          {STAT_CARDS.map((card) => (
            <div className="dueno-stat-card" key={card.label}>
              ...
              <p className="dueno-stat-value">{card.value}</p>
        ...
        <section className="dueno-alerts-section">
          <div className="dueno-alerts-header">
            ...
            <span className="dueno-alerts-count">{INVENTORY_ALERTS.length} productos</span>
          </div>
            ...
            <table className="dueno-table">
              ...
              <tbody>
                {INVENTORY_ALERTS.map((item) => (
                  <tr key={item.id} className={item.estado !== 'ok' ? 'dueno-row-alert' : ''}>
                    <td>{item.id}</td>
                    <td className="dueno-cell-strong">{item.producto}</td>
                    <td>{item.categoria}</td>
                    <td>
                      <div className="dueno-stock-cell">
                        <span className={`dueno-stock-number dueno-stock-${item.estado}`}>{item.stock}</span>
                        <div className="dueno-stock-track">
                          <div
                            className={`dueno-stock-fill dueno-stock-fill-${item.estado}`}
                    ...
                    <td>
                      <span className={`dueno-badge dueno-badge-${item.estado}`}>
